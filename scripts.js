...
function getSkillById(ids){
    ...
    if(skills_json != null){
        for(let i = 0; i < skills_json.content.length && skills.length != skills_json.content.length;i++){
            if(skills_json.content[i].id == id)skills.push(skills_json.content[i]);
        }
    }
    return skills;
}

function getBucketSkillsHTML(skills,selection,bucket=null,isInverse = false,renderFalseToo = false){
    let renderedHTML = "";
    let falseHTML = "";
    skills.forEach(skill =>{
        let is_in_bucket = bucket == null;
        if(!is_in_bucket)is_in_bucket = bucket.content.includes(skill);
        if(!is_in_bucket)return;
        ...
        if(is_rendered_if_render_only || renderFalseToo){
            ...
            skill_html = skill_html.replaceAll("{name}",skill.name);
            skill_html = skill_html.replaceAll("{stars}",getScoreHTML(skill.score));
            skill_html = skill_html.replaceAll("{desc}",skill.description);
            if(is_rendered_if_render_only)renderedHTML += skill_html;
            else falseHTML += skill_html;

            //Dynamical improvement + allow unselected ones at the end to be processed
            if(selection.includes(skill.id))selection.pop(skill.id);
        };
    });
    return [renderedHTML,falseHTML];
}

function getUnBucketedIDList(total_skills,buckets){
    let bucketed_ids = [];
    for(let i = 0; i < buckets.length;i++){
        bucketed_ids.concat(buckets[i].content);
    }

    let unbucketed_ids = [];
    total_skills.forEach(skill => {
        if(!bucketed_ids.includes(skill.id))unbucketed_ids.push(skill.id);
    });
...
async function insertSkills(){
    ...
    if(skill_template == ""){
        ...
        skill_template = skill_template_r.text();
        skill_group_template = skill_group_template_r.text();
    }
    if(skills_json == null){
        let skills_json_r = await fetch("json/skills.json");
        if(!skills_json_r.ok)throw new Error("Network error retrieving skills");
        skills_json = skills_json_r.json();
    }
    ...
    for(let i = 0; i < highlighted_buckets; i++){
        ...
        let group_highlighted_skills, group_unhighlighted_skills = getBucketSkillsHTML(total_skills,selected_skills_json,highlighted_buckets[i],false,true);

        group_template = group_template.replaceAll("{highlighted_skills}",group_highlighted_skills);
        group_template = group_template.replaceAll("{unhighlighted_skills}",group_unhighlighted_skills);
        highlighted_skills_html += group_template;
    }

    //Display highlighted skills not in a group
    let highlighted_other_skills, dump = getBucketSkillsHTML(total_skills,selected_skills,null,false,false);
    ...
    let remaining_selection = getUnBucketedIDList(highlighted_buckets);
    ...
    for(let i = 0; i < skills_json.buckets.length; i++){
        if(!highlighted_buckets.includes(skills_json.buckets[i])){
            ...
            let unhighlighted_group_skills, dump = getBucketSkillsHTML(total_skills,remaining_selection,skills_json.buckets[i],true,false);
            ...
            ungroup_template = ungroup_template.replaceAll("{highlighted_skills}",unhighlighted_group_skills);
            rest_of_skills_html += other_group;
        }
    }

    //Display the left over skills not in buckets or highlighted
    let unhighlighted_other_skills_html = getBucketSkillsHTML(total_skills,remaining_selection,null,false,false);
    let unhighlighted_other_group_skills_html = skill_group_template;
    ungroup_template = ungroup_template.replaceAll("{name}","Other");
    ungroup_template = ungroup_template.replaceAll("{score}","");
    ungroup_template = ungroup_template.replaceAll("{highlighted_skills}",unhighlighted_other_skills_html);
